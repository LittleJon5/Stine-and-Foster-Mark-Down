...
```{r}
ggplot(data = plot.table[plot.table$day.type == "Weekday",],  # the part in the [] subsets the data
                                                              # to just weekdays
       aes(x = gas.type, 
           y = value,
           fill = gas.type)) + # using a variable in the fill possition will give each gas.type a color
...
ggplot(data = plot.table[plot.table$gas.type == "Premium",], # this time we are subseting on premium gas
       aes(x = day.type,
           y = value,
           fill = day.type)) +
